...
export const Header: React.FC<HeaderProps> = ({
  cartCount, likeCount, activeSection,
  onNavigate, onOpenCart, onOpenFavorites,
  theme, onToggleTheme,
}) => {
  ...
  const handleCategory = (id: string) => {
    onNavigate("catalog");
    setCatalogOpen(false);
    // Передаём выбранную категорию через CustomEvent
  ...
  return (
    <header className={styles.header}>
      <div className={styles.inner}>
        ...
        {/* Логотип */}
        <button className={styles.logo} onClick={() => onNavigate("hero")}>
          <img
          ...
          />
          <span className={styles.logoText}><span className={styles.logoTKD}>TKD</span><span className={styles.logoEquip}>equip</span></span>
        </button>
        ...
        <nav className={styles.nav}>
          ...
          <div className={styles.catalogWrap} ref={catalogRef}>
            <button
              className={`${styles.navLink} ${activeSection === "catalog" ? styles.active : ""} ${catalogOpen ? styles.active : ""}`}
